...
Shader "VFX/Liquid_Shader_URP"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 _Texture_Normal_ST;
			float4 _Texture_ST;
			float _DistortionValue;
			float _Distortion_mult;
			float _Metallic;
			float _Smoothness;
			float _Emissive;
			float _Softness;
						 ...
						 ) : SV_Target
			{
				...
				float2 uv_Texture_Normal = input.ase_texcoord6.xyz.xy * _Texture_Normal_ST.xy + _Texture_Normal_ST.zw;
				float3 unpack128 = UnpackNormalScale( tex2D( _Texture_Normal, uv_Texture_Normal ), _DistortionValue );
				unpack128.z = lerp( 1, unpack128.z, saturate(_DistortionValue) );
				float3 tex2DNode128 = unpack128;
				...
				float4 fetchOpaqueVal177 = float4( SHADERGRAPH_SAMPLE_SCENE_COLOR( ( (tex2DNode128).xy + (ase_grabScreenPosNorm).xy ).xy ), 1.0 );
				#ifdef _SCREENCOLORTRUE_CUBEMAPFALSE_ON
				float4 staticSwitch269 = fetchOpaqueVal177;
				#else
				float4 staticSwitch269 = ( texCUBE( _CubeMap, worldRefl268 ) * 5 );
				#endif
				float4 lerpResult229 = lerp( input.ase_color , staticSwitch269 , _Distortion_mult);
				...
				float2 uv_Texture = input.ase_texcoord6.xyz.xy * _Texture_ST.xy + _Texture_ST.zw;
				float clampResult265 = clamp( tex2D( _Texture, uv_Texture ).r , 0.0 , 1.0 );
				float4 texCoord154 = input.ase_texcoord7;
				texCoord154.xy = input.ase_texcoord7.xy * float2( 1,1 ) + float2( 0,0 );
				

				float3 BaseColor = lerpResult229.rgb;
				float3 Normal = worldNormal262;
				float3 Specular = 0.5;
				float Metallic = _Metallic;
				float Smoothness = _Smoothness;
				float Occlusion = 1;
				float3 Emission = ( ( input.ase_color * _Emissive ) * clampResult231 ).rgb;
				float Alpha = saturate( ( ( clampResult265 - texCoord154.x ) / _Softness ) );
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float clampResult265 = clamp( tex2D( _Texture, uv_Texture ).r , 0.0 , 1.0 );
				float4 texCoord154 = input.ase_texcoord2;
				texCoord154.xy = input.ase_texcoord2.xy * float2( 1,1 ) + float2( 0,0 );
		...
		Pass
		{
			...
			half4 frag(PackedVaryings input  ) : SV_Target
			{
				...
				float2 uv_Texture_Normal = input.ase_texcoord3.xyz.xy * _Texture_Normal_ST.xy + _Texture_Normal_ST.zw;
				float3 unpack128 = UnpackNormalScale( tex2D( _Texture_Normal, uv_Texture_Normal ), _DistortionValue );
				unpack128.z = lerp( 1, unpack128.z, saturate(_DistortionValue) );
				float3 tex2DNode128 = unpack128;
				...
				float2 uv_Texture = input.ase_texcoord3.xyz.xy * _Texture_ST.xy + _Texture_ST.zw;
				float clampResult265 = clamp( tex2D( _Texture, uv_Texture ).r , 0.0 , 1.0 );
				float4 texCoord154 = input.ase_texcoord8;
				texCoord154.xy = input.ase_texcoord8.xy * float2( 1,1 ) + float2( 0,0 );
		...
		Pass
		{
			...
			half4 frag(PackedVaryings input  ) : SV_Target
			{
				...
				float2 uv_Texture_Normal = input.ase_texcoord1.xyz.xy * _Texture_Normal_ST.xy + _Texture_Normal_ST.zw;
				float3 unpack128 = UnpackNormalScale( tex2D( _Texture_Normal, uv_Texture_Normal ), _DistortionValue );
				unpack128.z = lerp( 1, unpack128.z, saturate(_DistortionValue) );
				float3 tex2DNode128 = unpack128;
				...
				float2 uv_Texture = input.ase_texcoord1.xyz.xy * _Texture_ST.xy + _Texture_ST.zw;
				float clampResult265 = clamp( tex2D( _Texture, uv_Texture ).r , 0.0 , 1.0 );
				float4 texCoord154 = input.ase_texcoord6;
				texCoord154.xy = input.ase_texcoord6.xy * float2( 1,1 ) + float2( 0,0 );
		...
		Pass
		{
						 ...
						 )
			{
				...
				float2 uv_Texture_Normal = input.ase_texcoord3.xy * _Texture_Normal_ST.xy + _Texture_Normal_ST.zw;
				float3 unpack128 = UnpackNormalScale( tex2D( _Texture_Normal, uv_Texture_Normal ), _DistortionValue );
				unpack128.z = lerp( 1, unpack128.z, saturate(_DistortionValue) );
				float3 tex2DNode128 = unpack128;
				float3x3 ase_tangentToWorldFast = float3x3( TangentWS.x, BitangentWS.x, NormalWS.x, TangentWS.y, BitangentWS.y, NormalWS.y, TangentWS.z, BitangentWS.z, NormalWS.z );
				...
				float clampResult265 = clamp( tex2D( _Texture, uv_Texture ).r , 0.0 , 1.0 );
				float4 texCoord154 = input.ase_texcoord4;
				texCoord154.xy = input.ase_texcoord4.xy * float2( 1,1 ) + float2( 0,0 );
		...
		Pass
		{
				 ...
				 ) : SV_Target
			{
				...
				surfaceDescription.Alpha = saturate( ( ( clampResult265 - texCoord154.x ) / _Softness ) );
...
Node;AmplifyShaderEditor.RangedFloatNode, AmplifyShaderEditor, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null;136;2688,1040;Inherit;False;Property;_Softness;Softness;4;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.LerpOp, AmplifyShaderEditor, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null;229;3184,256;Inherit;False;3;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;2;FLOAT;0;False;1;COLOR;0
...
Node;AmplifyShaderEditor.SaturateNode, AmplifyShaderEditor, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null;137;2960,944;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
...
WireConnection;235;1;262;0
WireConnection;235;3;258;0
WireConnection;235;4;261;0
WireConnection;235;2;255;0
WireConnection;235;6;137;0
ASEEND*/
//CHKSM=DB6AEE8109D232B042A8C30223589907CC13B21A